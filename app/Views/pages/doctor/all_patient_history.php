<?= view('components/doctor/navbar') ?>
<div class="flex flex-col gap-10 p-10 pt-30 justify-center items-center w-full h-screen font-changa bg-[#c9c9c9]">
    <div class="flex w-220 justify-between items-center">
        <a href="/dokter/dashboard"><button class="p-1 w-25 font-comic font-bold text-md text-white bg-[#1c80ad] cursor-pointer rounded-lg">Kembali</button></a>
        <div class="flex flex-col items-center p-5 bg-[#1c80ad] rounded-lg shadow-xl">
            <h1 class="text-3xl text-center text-shadow-sm text-shadow-white">Sudah Diperiksa</h1>
        </div>
        <div class="w-25"></div>
    </div>
    <div class="flex flex-col w-220 items-center p-10 bg-[#1c80ad] rounded-lg shadow-xl">
        <div class="flex flex-col w-full gap-5">
            <div class="flex justify-between">
                <h1 class="w-full text-center text-lg">Pasien</h1>
                <h1 class="w-full text-center text-lg">Tanggal</h1>
                <h1 class="w-full text-center text-lg">Keluhan</h1>
                <h1 class="w-full text-center text-lg">Diagnosa</h1>
                <h1 class="w-full text-center text-lg">Obat & Resep</h1>
            </div>
            <div class="flex justify-between font-comic font-bold">
                <h1 class="w-full text-center">Udin</h1>
                <h1 class="w-full text-center">02 September 2026</h1>
                <h1 class="w-full text-center">Sakit perut</h1>
                <h1 class="w-full text-center">TBC</h1>
                <h1 class="w-full text-center">Paracetamol 100x</h1>
            </div>
            <div class="flex justify-between font-comic font-bold">
                <h1 class="w-full text-center">Ucap</h1>
                <h1 class="w-full text-center">04 September 2026</h1>
                <h1 class="w-full text-center">Sakit gigi</h1>
                <h1 class="w-full text-center">Kanker</h1>
                <h1 class="w-full text-center">Obat Tidur 999x</h1>
            </div>
            <div class="flex justify-between font-comic font-bold">
                <h1 class="w-full text-center">Udah</h1>
                <h1 class="w-full text-center">17 Agustus 1945</h1>
                <h1 class="w-full text-center">Pusing</h1>
                <h1 class="w-full text-center">Patah kaki</h1>
                <h1 class="w-full text-center">Racun Sianida 100x</h1>
            </div>
        </div>
    </div>
</div>